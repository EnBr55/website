...
const colorToConcentration = (color) => {
  return (color[0]-30 + color[1]-30 + color[2]-30) / (225*3)
}

export default class Ant {
  constructor(x, y, trailLength, speed, followProb, followRate) {
    this.pos = {x: x, y: y}
    this.trailLength = trailLength
    this.trail = new Array(trailLength).fill({x: 0, y: 0})
  ...
  update(windowDimensions, index, samplePoints) {
    ...
    let probabilities = colors.map(a => colorToConcentration(a))
    let indexOfMaxValue = probabilities.reduce((iMax, x, i, arr) => x > arr[iMax] ? i : iMax, 0)
    probabilities[0] = colorToConcentration([255 - colors[0][0], 255 - colors[0][1], 255 - colors[0][2]])
  ...
  draw(drawFunc, getAverageColor) {
    ...
    for (let trailPoint of this.trail) {
      //drawFunc(getAverageColor(trailPoint.x, trailPoint.y), trailPoint.x, trailPoint.y, this.radius)
      drawFunc(this.color, trailPoint.x, trailPoint.y, this.radius)
    }
